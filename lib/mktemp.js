...
function createFile(template, mode, callback) {
  ...
  uniqueName(template, function(err, filename){
    if(err) { callback(err); return; }
    fs.open(filename, 'ax+', mode, function(err, fd) {
      if(err) { callback(err); return; }
...
function createFileSync(template, mode) {
  var filename = uniqueNameSync(template),
    fd = fs.openSync(filename, 'ax+', mode);
  fs.closeSync(fd);
  return filename;
}
